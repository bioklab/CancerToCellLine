...
# ##### Pick out three representive genesets #########
...
# 
# gs1 <- 'HALLMARK_COMPLEMENT'
# gs2 <- 'HALLMARK_PROTEIN_SECRETION'
# gs3 <- 'HALLMARK_E2F_TARGETS'
# 
...
#                   data.source = data.source,
#                   gene.set    = 'PROTEIN_SECRETION'
# )
# df3 <- data.frame(gsea.score = non.basal.gsea.results[gs3,],
#                   data.source = data.source,
#                   gene.set    = 'E2F_TARGETS'
# )
# df <- rbind(df1,df2,df3)
# df$data.source <- factor(df$data.source,levels = c('MET500','ORGANOIDS','CCLE'))
# ggplot(df) + geom_boxplot(aes(x=gene.set,color=data.source,y=gsea.score),size=1.5) + ggplot.style + scale_color_manual(values=c("MET500" =  "#800000", "ORGANOIDS" = "#008080","CCLE" = "#e6beff")) + ylab('ssGSEA.score')
# ggsave(filename = '/Users/liuke/OneDrive/OneDrive - Michigan State University/Project/Cancer2CellLine/Manuscript/section.Figure/Section5/non.basal.gene.set.box.plot.pdf',width=20,height=20)
...
# 
# 
# organoid.line              <- colnames(hallmark.geneset.gsea.results)[grepl(x=colnames(hallmark.geneset.gsea.results),pattern='ORGANOID')]
# organoid.dist              <- dist(hallmark.geneset.gsea.results[,organoid.line] %>% t,method='euclidean')
# organoid.hclust.rs         <- hclust(organoid.dist,method = 'complete')
# organoid.line              <- organoid.hclust.rs$labels[organoid.hclust.rs$order]
# 
# 
# 
# 
