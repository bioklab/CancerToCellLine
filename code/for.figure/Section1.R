...
###### attentation: Fig2a is revised in revision/round2.R
###### in the final submission, the TCGA mutation data is from GDC. So, I just keep the code here
###### the real figures and results reported in the paper are according based on the results of revision/round2.R
############################################################################################################
...
oncoPrint(pooled.mutation.profile[,c(MET500.rearranged.sample,CCLE.rearranged.sample)], get_type = function(x) {ifelse(x == 1,'MUT','background')},row_names_side = 'left',
          ...
          show_column_names = FALSE,show_row_barplot = F,top_annotation = col.annotation,show_pct = FALSE,row_names_gp = gpar(fontsize = 13,fontface=4)
) + 
  Heatmap(cbind(MET500.breast.cancer.gene.mutation.freq[pooled.gene],CCLE.breast.cancer.gene.mutation.freq[pooled.gene]),show_column_dend = FALSE, 
          ...
          row_names_gp = gpar(fontsize = 13,fontface=4),
...
Heatmap(log10(0.01+cbind(MET500.breast.cancer.gene.mutation.freq[CCLE.specific.highly.mutated.gene],TCGA.breast.cancer.gene.mutation.freq[CCLE.specific.highly.mutated.gene],CCLE.breast.cancer.gene.mutation.freq[CCLE.specific.highly.mutated.gene]) ),show_heatmap_legend = FALSE, 
        ...
        col=colorRamp2(c(-2,-0.05),c('blue','red')),row_names_gp = gpar(fontsize = 35,fontface=4),
        width = unit(20, "in")
)
dev.off()
...
##############################################
##### Table 1: mutation frequency of  DE mutated genes  (well, Table 1 will not be mentioned in the manuscript, just be used to keep tracking of DE genes) 
...
##############################################
# well,actually Table1 is not included in the manuscript, but just keep it here 
