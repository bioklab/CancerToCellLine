...
# a=EMT.ssgsea.score[CCLE.breast.cancer.cell.line]
# b=y[CCLE.breast.cancer.cell.line]
# draw.df <- data.frame(x=a,y=b,cell.line.name=names(b))
# ggplot(draw.df) + geom_label(aes(x=x,y=y,label=cell.line.name))
# 
# 
# 
# y                <- MET500.vs.CCLE.polyA.LumB.expression.correlation.result$cell.line.median.cor
# plot(x=EMT.ssgsea.score[CCLE.breast.cancer.LumB.cell.line],y=y[CCLE.breast.cancer.LumB.cell.line])
# 
# 
# y                <- MET500.vs.CCLE.polyA.LumA.expression.correlation.result$cell.line.median.cor
# plot(x=EMT.ssgsea.score[CCLE.breast.cancer.LumA.cell.line],y=y[CCLE.breast.cancer.LumA.cell.line])
# 
# y                <- MET500.vs.CCLE.polyA.Her2.expression.correlation.result$cell.line.median.cor
# plot(x=EMT.ssgsea.score[CCLE.breast.cancer.Her2.cell.line],y=y[CCLE.breast.cancer.Her2.cell.line])
# 
...
# y <- MET500.vs.CCLE.polyA.Basal.expression.correlation.result$cell.line.median.cor[CCLE.breast.cancer.Basal.cell.line]
# flag <- (names(y) %in% colnames(BRACA.log2.fpkm.matrix))
# high.cor.cell.line <- names(y)[flag]
# high.cor.cell.line.krt14.expr <- BRACA.log2.fpkm.matrix[krt14,]
# df5 <- data.frame(expr=MDAMB231.krt14.expr,subtype='MDA-MB-231')
...
# BRACA.log2.fpkm.matrix[krt14,'MDAMB231_BREAST']
# BRACA.log2.fpkm.matrix[krt14,'HCC70_BREAST']
# BRACA.log2.fpkm.matrix[krt14,'HCC2157_BREAST']
# BRACA.log2.fpkm.matrix[krt14,'HCC1569_BREAST']
# BRACA.log2.fpkm.matrix[krt14,'HDQP1_BREAST']


# require(GEOquery)
# gse.id <- 'GSE57544'
# gse.obj <- getGEO(gse.id,GSEMatrix = FALSE)
# expr.matrix <- foreach(gsm.obj = gse.obj@gsms,.combine='cbind') %do% {
#     x@dataTable@table[,'VALUE']  %>% as.numeric
# }
# probe.name <- gse.obj@gsms[[1]]@dataTable@table[,'ID_REF'] %>% as.character
# 
# sample.name<- foreach(gsm.obj = gse.obj@gsms,.combine='c') %do% {
#     s@header$geo_accession 
# }
# 
# rownames(expr.matrix) <- probe.name
# colnames(expr.matrix) <- sample.name
# 
# illumina.microarray.probe.2.gene <- read.delim("~/Project/Cancer2CellLine/client-side/meta.data/illumina.microarray.probe.2.gene.txt", stringsAsFactors=FALSE)
# flag <- illumina.microarray.probe.2.gene$ILLUMINA.HumanHT.12.V4.probe == ''
# illumina.microarray.probe.2.gene <- illumina.microarray.probe.2.gene[!flag,]
# 
# expr.matrix <- expr.matrix[rownames(expr.matrix) %in% illumina.microarray.probe.2.gene$ILLUMINA.HumanHT.12.V4.probe,]
# ensemble.id <- illumina.microarray.probe.2.gene$Gene.stable.ID[match(rownames(expr.matrix),illumina.microarray.probe.2.gene$ILLUMINA.HumanHT.12.V4.probe)]
# 
# require(limma)
# expr.matrix <- avereps(x = expr.matrix,ID=ensemble.id)
# 
# GSE57544.obj <- pick.out.cell.line(expr.of.samples = expr.matrix,expr.of.cell.lines = CCLE.log2.rpkm.matrix,marker.gene = CCLE.rna.seq.marker.gene.3000)
# View(GSE57544.obj$cell.line.median.cor)
