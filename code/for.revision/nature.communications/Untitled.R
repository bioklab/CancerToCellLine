...
s                   <- intersect(rownames(BRACA.log2.fpkm.matrix),rownames(organoid.log2.rpkm.matrix))
s                   <- intersect(rownames(CCLE.log2.rpkm.matrix),s)
data                <- CCLE.log2.rpkm.matrix[s,]
colnames(data)      <- paste(colnames(data),'_RAW_CCLE',sep='')

merged.model.matrix <- cbind(BRACA.log2.fpkm.matrix[s,],organoid.log2.rpkm.matrix[s,],data[s,]
                             #,
                             #CCLE.log[s,c(MET500.breast.cancer.polyA.LumA.sample,MET500.breast.cancer.polyA.LumB.sample,MET500.breast.cancer.polyA.Her2.sample,MET500.breast.cancer.polyA.Basal.sample)]
                             )

pca.rs <- prcomp(merged.model.matrix %>% t)
pairs(pca.rs$x[,1:5])
pc1 <- pca.rs$x[,1]
...
ggsave.exp(filename = '/Users/liuke/OneDrive/OneDrive - Michigan State University/Project/Cancer2CellLine/Manuscript/section.Figure/Section4/Basal.organoid.vs.cell.line.population.pdf',width=20,height=15)
...
CCLE.breast.cancer.non.Basal.cell.line <- CCLE.breast.cancer.non.Basal.cell.line[CCLE.breast.cancer.non.Basal.cell.line %in% colnames(BRACA.log2.fpkm.matrix)]
...
ggplot(rbind(df1,df2),aes(x=type,y=median.cor)) + geom_boxplot(outlier.shape=NA,lwd=1.2) + geom_point(size=4,position=position_jitterdodge(jitter.width = 0.2),aes(fill=type)) + ggplot.style + xlab('') + ylab('transcriptome similarity')
#ggsave.exp.exp(filename = '/Users/liuke/OneDrive/OneDrive - Michigan State University/Project/Cancer2CellLine/Manuscript/section.Figure/Section4/non.Basal.organoid.vs.cell.line.population.pdf',width=20,height=15)
wilcox.test(df1$median.cor,df2$median.cor,paired = FALSE)
...
#ggsave.exp.exp(filename = '/Users/liuke/OneDrive/OneDrive - Michigan State University/Project/Cancer2CellLine/Manuscript/section.Figure/Section4/LumA.pdf',width=20,height=15)
...
ggplot(rbind(df1,df2),aes(x=type,y=correlation)) + geom_boxplot(outlier.shape=NA,lwd=1.2) + geom_point(size=4,position=position_jitterdodge(jitter.width = 0.2),aes(fill=type)) + ggplot.style + xlab('') + ylab('transcriptome similarity')
#ggsave.exp.exp(filename = '/Users/liuke/OneDrive/OneDrive - Michigan State University/Project/Cancer2CellLine/Manuscript/section.Figure/Section4/LumB.pdf',width=20,height=15)
...
ggplot(rbind(df1,df2),aes(x=type,y=correlation)) + geom_boxplot(outlier.shape=NA,lwd=1.2) + geom_point(size=4,position=position_jitterdodge(jitter.width = 0.2),aes(fill=type)) + ggplot.style + xlab('') + ylab('transcriptome similarity')
ggsave.exp.exp(filename = '/Users/liuke/OneDrive/OneDrive - Michigan State University/Project/Cancer2CellLine/Manuscript/section.Figure/Section4/Her2.pdf',width=20,height=15)
...
ggplot(rbind(df1,df2),aes(x=type,y=correlation)) + geom_boxplot(outlier.shape=NA,lwd=1.2) + geom_point(size=4,position=position_jitterdodge(jitter.width = 0.2),aes(fill=type)) + ggplot.style + xlab('') + ylab('transcriptome similarity')
ggsave.exp(filename = '/Users/liuke/OneDrive/OneDrive - Michigan State University/Project/Cancer2CellLine/Manuscript/section.Figure/Section4/Basal.pdf',width=20,height=15)
wilcox.test(df1$correlation,df2$correlation,paired = TRUE)



# ## Pick out good organoid lins
# CCLE.non.breast.cancer.cell.line <- setdiff(names(MET500.vs.CCLE.and.organoid.polyA.LumB.expression.correlation.result$cell.line.median.cor),c(CCLE.breast.cancer.cell.line,organoid.line))
# 
# 
...
# p.value                          <- pnorm(q=MET500.vs.CCLE.and.organoid.polyA.Basal.expression.correlation.result$cell.line.median.cor[organoid.line],mean = m,sd = s,lower.tail = FALSE) # compute p-value
...
# p.value                          <- pnorm(q=MET500.vs.CCLE.and.organoid.polyA.LumA.expression.correlation.result$cell.line.median.cor[organoid.line],mean = m,sd = s,lower.tail = FALSE) # compute p-value
# fdr.vec                          <- p.adjust(p.value,method='fdr')
# MET500.LumA.good.organoid        <- names(fdr.vec)[fdr.vec <= 0.01]
# 
# m                                <- MET500.vs.CCLE.and.organoid.polyA.LumB.expression.correlation.result$cell.line.median.cor[CCLE.non.breast.cancer.cell.line] %>% median
# s                                <- MET500.vs.CCLE.and.organoid.polyA.LumB.expression.correlation.result$cell.line.median.cor[CCLE.non.breast.cancer.cell.line] %>% mad                  # fit the normal distribtuion 
# p.value                          <- pnorm(q=MET500.vs.CCLE.and.organoid.polyA.LumB.expression.correlation.result$cell.line.median.cor[organoid.line],mean = m,sd = s,lower.tail = FALSE) # compute p-value
# fdr.vec                          <- p.adjust(p.value,method='fdr')
# MET500.LumB.good.organoid        <- names(fdr.vec)[fdr.vec <= 0.01]
# 
# m                                <- MET500.vs.CCLE.and.organoid.polyA.Her2.expression.correlation.result$cell.line.median.cor[CCLE.non.breast.cancer.cell.line] %>% median
# s                                <- MET500.vs.CCLE.and.organoid.polyA.Her2.expression.correlation.result$cell.line.median.cor[CCLE.non.breast.cancer.cell.line] %>% mad                  # fit the normal distribtuion 
# p.value                          <- pnorm(q=MET500.vs.CCLE.and.organoid.polyA.Her2.expression.correlation.result$cell.line.median.cor[organoid.line],mean = m,sd = s,lower.tail = FALSE) # compute p-value
# fdr.vec                          <- p.adjust(p.value,method='fdr')
# MET500.Her2.good.organoid        <- names(fdr.vec)[fdr.vec <= 0.01]
